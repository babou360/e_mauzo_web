...
export default function DeletedProducts() {
  ...
  const handleRestore = (business:Product) => {
  ...
  return (
    <div className={styles.deletedBusiness}>
      ...
      {prodLoading ? (
        ...
        >
          <AlertCircle size={56} />
          <h3>{t('No Deleted Businesses', 'Hakuna Bidhaa Zilizofutwa')}</h3>
          <p>{t('All your businesses are active.', 'Bidhaa zote ziko hai.')}</p>
        </motion.div>
      ) : (
        /* Business List */
        <motion.div
        ...
        >
          <AnimatePresence>
            {products.data.map((business:any, i:number) => (
              <motion.div
                key={business.id}
