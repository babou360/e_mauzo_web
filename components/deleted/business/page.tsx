...
export default function DeletedBusiness() {
  ...
  const [showConfirmModal, setShowConfirmModal] = useState<Business | null>(null);
  ...
  const handleRestore = (business:Business) => {
  ...
  return (
    <div className={styles.deletedBusiness}>
      ...
      {biziLoading ? (
        ...
        >
          <AlertCircle size={56} />
          <h3>{t('No Deleted Businesses', 'Hakuna Biashara Zilizofutwa')}</h3>
          <p>{t('All your businesses are active.', 'Biashara zote ziko hai.')}</p>
        </motion.div>
      ) : (
        /* Business List */
        <motion.div
          ...
          <div className={styles.modalOverlay} onClick={() => setShowConfirmModal(null)}>
            ...
            >
              <div className={styles.modalHeader}>
                <h2>{t('Restore Business?', 'Kurejesha Biashara?')}</h2>
                <button
