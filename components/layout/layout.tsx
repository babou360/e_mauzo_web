...
//   // Don't show footer on sign_log
//   const showFooter = !isFullScreen && pathname !== '/sign_log';
...
//             <div className={cx(styles.pageContent, { [styles.noFooter]: !showFooter })}>
...
//             {/* Footer - only show on non-fullscreen pages and not on sign_log */}
//             {showFooter && (
//               <footer className={styles.footer}>
//                 <div className={styles.footerContent}>
//                   <div className={styles.footerSection}>
//                     <h3 className={styles.footerLogo}>Bazenga</h3>
//                     <p className={styles.footerVersion}>Version 1.0.0</p>
//                     <p className={styles.footerCopyright}>
//                       &copy; {new Date().getFullYear()} Bazenga. {language === "Swahili" ? "Haki zote zimehifadhiwa" : "All rights reserved."}
//                     </p>
//                   </div>
                  
//                   <div className={styles.footerSection}>
//                     <h4 className={styles.footerTitle}>{language === "Swahili" ? "Viungo vya Kijamii" : "Social Links"}</h4>
//                     <div className={styles.socialLinks}>
//                       <a href="https://instagram.com/swahilicodes" target="_blank" rel="noopener noreferrer" className={styles.socialLink}>
//                         <FaInstagram />
//                       </a>
//                       <a href="https://tiktok.com/swahilicodes" target="_blank" rel="noopener noreferrer" className={styles.socialLink}>
//                         <FaTiktok />
//                       </a>
//                       <a href="https://linkedin.com/company/bazenga" target="_blank" rel="noopener noreferrer" className={styles.socialLink}>
//                         <FaLinkedin />
//                       </a>
//                     </div>
//                   </div>
...
const LayoutComponent = ({ children }: { children: React.ReactNode }) => {
  ...
  const [buttonCycleTimer, setButtonCycleTimer] = useState<NodeJS.Timeout | null>(null);

  // Contact information
  const contactInfo = {
    phone: '+255785008525',
    email: 'leebabou@gmail.com',
    address: 'chama, kibamba, Ubungo, Dar-es-salaam, Tanzania'
  };
  ...
  useEffect(() => {
    ...
    if (!bizLoading && businesses !== undefined) {
      ...
      if (businesses && businesses.length > 0 && savedBusiness) {
        ...
        // Allow navigation to allowed pages only
        const allowed = ['/', '/sales', '/products', '/attendants', '/clients', '/expenses', '/damages', '/business', '/deleted', '/businesses', '/business-reports','/terms-and-conditions'];
        if (!allowed.includes(pathname)) {
          safeRedirect('/');
          return;
        }
  ...
  // Floating button visibility - always show on dashboard pages
  useEffect(() => {
    // Don't show floating button on fullscreen pages or sign_log
    const fullScreenPages = ['/sign_log', '/user/verify', '/subscription', '/first_screen', '/terms-and-conditions', '/choose_business', '/create_business'];
    const isFullScreen = fullScreenPages.includes(pathname);
    
    // Show button only on dashboard pages (not on fullscreen or sign_log)
    if (!isFullScreen && pathname !== '/sign_log') {
      setShowFloatingButton(true);
    } else {
      setShowFloatingButton(false);
      setShowButtonText(false); // Reset text state when button is hidden
    }
  }, [pathname]);

  // Cycle text animation for floating button
  useEffect(() => {
    if (!showFloatingButton) return;

    // Start with text hidden (icon only)
    setShowButtonText(false);
    
    // Clear any existing timer
    if (buttonCycleTimer) clearTimeout(buttonCycleTimer);
    
    // Function to cycle text visibility
    const cycleText = () => {
      setShowButtonText(prev => !prev);
    };
    
    // First show text after 10 seconds
    const firstTimer = setTimeout(() => {
      ...
      setButtonCycleTimer(intervalTimer as unknown as NodeJS.Timeout);
    }, 10000);
    
    setButtonCycleTimer(firstTimer as unknown as NodeJS.Timeout);
    
    // Cleanup
    return () => {
      if (firstTimer) clearTimeout(firstTimer);
      if (buttonCycleTimer) clearInterval(buttonCycleTimer as unknown as NodeJS.Timeout);
    };
  }, [showFloatingButton, language]);
        ...
        <main className={cx(styles.content, { [styles.fullWidth]: isFullScreen })}>
          ...
          <div className={styles.mainWrapper}>
            <div className={cx(styles.pageContent, { [styles.noFooter]: !showFooter })}>
              ...
              {/* Floating Button - Always visible with cycling text animation */}
              {showFloatingButton && !isFullScreen && pathname !== '/sign_log' && (
                <motion.div 
                ...
                >
                  ...
                  <AnimatePresence mode="wait">
                    {showButtonText && (
                      <motion.span 
                        key={language}
                        className={styles.floatingButtonText}
                      ...
                      >
                        {language === "Swahili" ? "Mawasiliano" : "Contact"}
                      </motion.span>
                    )}
                  </AnimatePresence>
                </motion.div>
      ...
      <AnimatePresence>
        {showContactModal && (
          ...
          >
            ...
            >
              <button className={styles.contactModalClose} onClick={handleCloseContactModal}>
                <X size={24} />
              </button>
              
              <div className={styles.contactModalHeader}>
                <h2>{language === "Swahili" ? "Mawasiliano Yetu" : "Contact Information"}</h2>
                <p>{language === "Swahili" ? "Wasiliana nasi kwa huduma yoyote" : "Reach out to us for any assistance"}</p>
              </div>
              ...
              <div className={styles.contactModalContent}>
                <div className={styles.contactInfoItem}>
                  ...
                  <div className={styles.contactInfoDetails}>
                    <h4>{language === "Swahili" ? "Simu" : "Phone"}</h4>
                    <a href={`tel:${contactInfo.phone}`}>{contactInfo.phone}</a>
                  </div>
                </div>
                
                <div className={styles.contactInfoItem}>
                  <div className={styles.contactInfoIcon}>✉️</div>
                  <div className={styles.contactInfoDetails}>
                    <h4>{language === "Swahili" ? "Barua Pepe" : "Email"}</h4>
                ...
                <div className={styles.contactInfoItem}>
                  <div className={styles.contactInfoIcon}>📍</div>
                  <div className={styles.contactInfoDetails}>
                    <h4>{language === "Swahili" ? "Anwani" : "Address"}</h4>
              ...
              <div className={styles.contactModalFooter}>
                <button onClick={handleCloseContactModal} className={styles.contactModalButton}>
